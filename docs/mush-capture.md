...
3. 页面右上角应出现 **WSBBB 副本地图探索采集器 2.1.0** 面板。
4. 若仍没有面板，在原版页面控制台执行：
...
需要交互才能开启通路的副本，不要让脚本自动猜测动作。先暂停自动探索，在房间内手动点击一个动作，等出口刷新后再继续并导出；对应房间的 `commands`、`exit_history` 和 `frames` 会保留动作及出口变化，便于在项目房间脚本中复现 `add_action`/`add_exit` 逻辑。
