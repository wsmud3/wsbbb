...
3. 页面右上角应出现 **WSBBB 副本地图探索采集器 2.1.2** 面板。
4. 若仍没有面板，在原版页面控制台执行：
...
- **导出**：下载 `mush-map-capture-*.json`。文件中的 `rooms` 是已探索的房间快照，`commands` 是房间动作按钮，`exit_history` 记录交互前后出口变化及最近触发命令，`frames` 是原始接口消息。

探索器会跳过 `out` 出口，避免自动离开副本；兼容原框架实际使用的短方向（`e/w/n/s`、斜向、上下组合）和长方向，每个出口都会独立尝试一次，不依赖出口值是否是标准房间路径。若抵达已有房间，则自动按反方向回到上一个房间继续尝试其余出口；不可达出口、战斗或没有回退方向时会停止/回退并保留已采集数据。为防止异常循环，单次最多采集 500 个房间。

需要交互才能开启通路的副本，不要让脚本自动猜测动作。先停止自动探索，在房间内手动点击一个动作，等出口刷新后再导出；即使探索器已停止，对应房间的 `commands`、`exit_history` 和 `frames` 仍会保留动作及出口变化，便于在项目房间脚本中复现 `add_action`/`add_exit` 逻辑。
