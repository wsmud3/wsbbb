...
(function () {
  ...
  function stepExplorer() {
    ...
    // Try every real direction once. Exit values vary between server versions
    // (path, display name, or empty), so they must not be used to skip an exit.
    // If the destination was already visited, beginRoom() sends the reverse move.
    const candidate = (frame.exits || []).find(exit => (
      !frame.tried[exit.direction] && exit.direction !== "out"
    ));
    if (candidate) {
